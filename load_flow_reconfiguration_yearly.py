...
def perform_load_flow(branches, new_branches_data, variations):
    
    results_volt=[]
    for active_load in variations:
        ...
        #active_load=variations[0]
        
        reactive_power_series = []
        for active_power in active_load:
            apparent_power = active_power / 0.9
            reactive_power = math.sqrt(apparent_power**2 - active_power**2)
            reactive_power_series.append(reactive_power)
        
        #active_load=load_data['P (kW)']
        #reactive_load=load_data['Q (kW)']
        
    
        reactive_load=reactive_power_series
