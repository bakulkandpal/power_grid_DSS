# Yearly network reconfiguration. Based on yearly network problems.
import pandas as pd
...
from load_flow_reconfiguration_yearly import perform_load_flow
...
#original_load_data = df[load_column].to_numpy()  

# Variation Function
def apply_variation(active_load, variation_factor=0.2):
    variation = np.random.uniform(-variation_factor, variation_factor, size=len(active_load))
    adjusted_loads = active_load * (1 + variation)
    return adjusted_loads  

# Generate Variations
variations = []
for _ in range(13):  ## Length of Simulated Days
    varied_loads = apply_variation(active_load)
    variations.append(varied_loads)
...
for line_to_remove, line_to_add in successful_combinations:
    ...
    results_volt = perform_load_flow(new_branches_list, new_branches_data, variations)      
    results.append((results_volt))
   
min_values = []
for outer_list in results:  
    inner_min_values = []
    for middle_list in outer_list:  
        volt_cons=middle_list[0]
        min_value = min(volt_cons)
        inner_min_values.append(min_value)

    min_values.append(inner_min_values)        

overall_min_values = []
for inner_list in min_values:
    min_value_all = min(inner_list)  
    overall_min_values.append(min_value_all)
        
max_value = max(overall_min_values)
max_index = overall_min_values.index(max_value)
print("Scenario with best voltage profiles:", max_index) 
